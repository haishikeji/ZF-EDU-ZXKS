<template>
  <a-drawer width="50%" :label-col="4" :wrapper-col="14" :visible="open" @close="onClose">
    <a-divider orientation="left">
      <b v-if="form.objTopic">{{ form.objTopic.topic }} (得分：{{form.totalPoints}}分)</b>
    </a-divider>
    <a-form-model ref="form" :model="form" :rules="rules">
        <div v-if="form.answerList" style="width:100%;">
          <div v-for="(item,index) in form.answerList" :key="index" style="margin-bottom:20px;margin-left:6%;">
              <b v-if="item.objIssue.type == 'dx'">{{index+1}}.【{{typeFormat(item.objIssue.type)}}】{{item.objIssue.issue}}({{dsF}}分)</b>
              <b v-if="item.objIssue.type == 'dxs'">{{index+1}}.【{{typeFormat(item.objIssue.type)}}】{{item.objIssue.issue}}({{dxsF}}分)</b>
              <b v-if="item.objIssue.type == 'pd'">{{index+1}}.【{{typeFormat(item.objIssue.type)}}】{{item.objIssue.issue}}({{pdF}}分)</b>
...
<script>
  import { getTopicById} from '@/api/obj/examine'
  export default {
    name: 'viewForm',
    props: {},
    components: {},
    ...
    typeFormat(type){
      ...
      handleShow(row, totalPoints) {
        ...
        getTopicById(id).then(response => {
          ...
          this.form.objTopic.sourceList.forEach(e => {
            if(e.type=='ds'){
              this.dsF = e.source;
            }
            if(e.type=='dxs'){
